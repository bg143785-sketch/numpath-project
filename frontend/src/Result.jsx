...
const Result = ({ formData }) => {
  ...
  if (!formData || !formData.name || !formData.day || !formData.month || !formData.year) {
    return (
      <div className="result-container">
        <div className="result-content">
          ...
          <div className="result-header">
            ...
            <p className="result-subtitle">Please fill the form first.</p>
          </div>
        </div>
      </div>
    );
  ...
  const calculateBirthNumber = (day) => {
    return reduceToSingleDigit(parseInt(day));
  };

  const calculateLifePathNumber = (day, month, year) => {
    return reduceToSingleDigit(parseInt(day) + parseInt(month) + parseInt(year));
  };

  const calculatePersonalYear = (day, month, year) => {
    return reduceToSingleDigit(parseInt(day) + parseInt(month) + year);
  };

  const calculatePersonalMonth = (year, month) => {
    return reduceToSingleDigit(year + month);
  };

  const calculatePersonalDay = (month, day) => {
